<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <title>SaiLaxmi Bank</title>
    <link rel="stylesheet" href="css/footer.css">
     <link rel="stylesheet" href="index.css">
</head>
<body>
<header>
    <nav class="navbar navbar-dark bg-dark">
         
    <h1>Sai Laxmi Bank</h1>
    <a href="index.php" class="Home_link" target="blank">
      <h4 >Home</h4>
    </a>
    <a href="customers.php" class="Customers_Info_link" target="blank">
      <h4>View Customers</h4>
      </a>
    <a href="customers.php" class="Make_Transaction_link" target="blank">
      <h4> Make transaction</h4>
    </a>
    <a href="transaction_history.php" class="Transaction_History_link" target="blank">

      <h4>Transaction History</h4>
    </a>
      </nav>
</header>
    <div class="bankname">
        <h1 class="display-1">SAI LAXMI BANK</h1>
    </div>
    <div class="welcomesyou">
        <h4 class="display-4">Welcomes You!!!</h4>
        <h6 class="Create">Created By- Tanya Bhardwaj.</h6>
        <h6 class="Project">A Spark Foundation Project</h6>
    </div>
    <div class="backimage">
        <img src="images/Bank Image.jpg" alt="Bank" height="200px" width="100%">
    </div>
    <div class="Customers">
        <img src="images/customer.jpg" alt="Customer">
        <p>CUSTOMER INFO</p> 
        <a href="customers.php" target="blank">
        <button type="button" class="CustomerInfo">Click It</button>
        </a>
    </div>
    <div class="Transaction">
        <img src="images/make transactions.jpg" alt="Make Transaction">
        <p>MAKE TRANSACTION</p>
        <a href="make_transaction.php" target="blank">
            <button type="button" class="CustomerInfo">Click It</button>
        </a>
    </div>
    <div class="TransactionHistory">
        <img src="images/transactions.jpg" alt="Transactions">
        <p>TRANSACTION HISTORY</p>
        <a href="transaction_history.php" target="blank">
            <button type="button" class="CustomerInfo">Click It</button>
        </a>
    </div>
    <footer id="footer" class="footer-1">
        <div class="main-footer widgets-dark typo-light">
        <div class="container">
        <div class="row">
          
        <div class="col-xs-12 col-sm-6 col-md-3">
        <div class="widget subscribe no-box">
        <h5 class="widget-title">SAI LAXMI BANK<span></span></h5>
        <p>The Sai Laxmi Bank provides internet based banking services.</p>
        </div>
        </div>
        
        <div class="col-xs-12 col-sm-6 col-md-3">
        <div class="widget no-box">
        <h5 class="widget-title">Quick Links<span></span></h5>
        <ul class="thumbnail-widget">
        <li>
        <div class="thumb-content"><a href="customers.php" target="blank">Customers Information</a></div>
        </li>
        <li>
        <div class="thumb-content"><a href="make_transaction.php" target="blank">Make Transactions</a></div>
        </li>
        <li>
        <div class="thumb-content"><a href="transaction_history.php" target="blank">Transaction History</a></div>
        </li>
        </ul>
        </div>
        </div>
        
        
        
        <div class="col-xs-12 col-sm-6 col-md-3">
        
        <div class="widget no-box">
        <h5 class="widget-title">Contact Us<span></span></h5>
        
        <p><a href="mailto:info@domain.com" title="glorythemes">voidme46@gmail.com</a></p>
        <ul class="social-footer2">
        <li class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a></li>
        <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a></li>
        <li class=""><a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a></li>
        </ul>
        </div>
        </div>
        </div>
        </div>
        </div>
          
        <div class="footer-copyright">
        <div class="container">
        <div class="row">
        <div class="col-md-12 text-center">
        <p>Copyright Sai Laxmi Bank © 2022. All rights reserved.</p>
        </div>
        </div>
        </div>
        </div>
        </footer>
</body>
</html>